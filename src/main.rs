...
/// Lokasi data default: `~/.aksara`. Vault & state Tor disimpan di sini supaya
/// `aksara` dari folder mana pun membuka identitas yang sama (zero-setup).
fn default_data_dir() -> PathBuf {
    home_dir().join(".aksara")
}
...
fn help_text() -> String {
    "\
AKSARA — serverless encrypted P2P terminal chat
...
Opsi:
  --vault <path>          Lokasi file vault (default: ~/.aksara/id.key)
...
badge berubah jadi TOR+LAN saat siap. Passphrase id dibaca dari env AKSARA_PASSPHRASE."
        .to_string()
}
...
fn read_passphrase(prompt: &str) -> Result<String, Error> {
    if let Ok(p) = std::env::var("AKSARA_PASSPHRASE") {
        return Ok(p);
    }
    print!("{prompt}");
    io::stdout().flush().ok();
...
async fn real_main(args: Args) -> Result<(), Error> {
    // Pastikan folder data (mis. ~/.aksara) ada sebelum tulis vault / state Tor.
    ...
    if args.print_id {
        let onion = if online {
            ...
            match TorContext::launch(&cache_dir, &state_dir, "aksara-room").await {
        ...
        println!();
        println!("  AKSARA  —  secure · serverless · sovereign");
        println!("  ─────────────────────────────────────────────");
        println!();
        println!("  Invite code:");
        println!("  {}", keys.invite);
    ...
    let tor_rx = if online {
        ...
        tokio::spawn(async move {
            let result = TorContext::launch(&cache_dir, &state_dir, "aksara-room")
                .await
