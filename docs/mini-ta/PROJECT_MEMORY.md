...
- TAHAP 1 s.d. 12 seluruhnya berstatus `DONE` (TAHAP 12 sempat `PARTIAL` menunggu screenshot manual, kini `DONE` penuh sejak 2026-07-30 — lihat entri screenshot di bawah).
...
- **Yang tersisa sebelum penyusunan DOCX**: seluruh screenshot (Sesi A dan Sesi B) sudah selesai sejak 2026-07-30 — tidak ada lagi item screenshot tertunda. Tersisa hanya dua pertanyaan kosmetik (apakah footer institusi dan Abstrak dua bahasa tetap diminta untuk tugas mata kuliah).
...
- Status screenshot TAHAP 12 (diperbarui 2026-07-30): **SELESAI PENUH, SESI A dan SESI B**. Sesi A: 5 berkas (`01-antarmuka-utama`, `02-identitas-invite`, `03-komunikasi-loopback-a/b`, `04-verifikasi-vault`). Sesi B (Tor, 2 laptop jaringan berbeda): 6 berkas (`05-onion-invite`, `06-tor-online`, `07a/07b-komunikasi-tor-laptop1/2`, `07c-bukti-jaringan-berbeda-laptop1/2` — Wi-Fi `192.168.102.128/24` vs `192.168.93.113/22`, subnet berbeda terkonfirmasi). Seluruhnya diambil dari binary rilis v0.2.1 pada Windows 11 PowerShell. Detail di `screenshots/STATUS.md`. TAHAP 12 kini `DONE` penuh, bukan lagi `PARTIAL` — tidak ada lagi capture screenshot yang tertunda.
- **Tiga hal yang WAJIB diperhatikan saat mengutip screenshot** (detail di `11_FIGURE_MANIFEST.md` §Daftar Screenshot): (a) jalur transport pada `03-*` adalah **TCP loopback**, bukan LAN fisik atau Tor — aplikasi tidak mencetak jalur transport di layar sehingga keterangan gambar wajib menyebutnya; (b) `04-verifikasi-vault.png` menampilkan passphrase terbaca, keduanya dummy sehingga bukan kebocoran, dan gambar itu sekaligus bukti visual atas keterbatasan terdokumentasi bahwa input passphrase stdin masih ter-echo (`08_THREAT_MODEL.md` §4.6, `PLANNED` M4) — manfaatkan, jangan sembunyikan; (c) pemangkasan area kosong gambar diizinkan, mengubah isi layar tidak.
- **Badge transport TUI** (dikonfirmasi dari `src/tui/ui.rs:311-317`): `⌂ LOCAL` abu-abu bila Tor mati, `LINKING` dengan spinner saat bootstrap, `◉ ONLINE` warna aksen saat Tor aktif. Tidak ada badge bertuliskan "TOR" atau "LAN" — jangan salah kutip.
