...
- **Yang tersisa sebelum penyusunan DOCX**: screenshot **Sesi B saja** (Tor, 2 laptop jaringan berbeda — Sesi A sudah selesai), plus dua pertanyaan kosmetik (apakah footer institusi dan Abstrak dua bahasa tetap diminta untuk tugas mata kuliah).
...
- Status screenshot TAHAP 12 (diperbarui 2026-07-29): **SESI A SELESAI** — 5 berkas PNG sudah masuk `01-claude-preparation/screenshots/` (`01-antarmuka-utama`, `02-identitas-invite`, `03-komunikasi-loopback-a/b`, `04-verifikasi-vault`), diambil dari binary rilis v0.2.1 pada Windows 11 PowerShell. **SESI B BELUM** — perlu 2 laptop di jaringan berbeda untuk membuktikan Tor (`05-onion-invite`, `06-tor-online`, `07a/b/c-komunikasi-tor`). Prosedur di `screenshots/PANDUAN_SCREENSHOT.md`. Tidak menghalangi `ready_for_codex`.
