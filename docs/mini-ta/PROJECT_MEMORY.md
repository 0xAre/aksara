...
- **Aturan format DOCX sudah diekstrak** ke `01-claude-preparation/16_DOCUMENT_FORMAT_SPEC.md` (2026-07-27): A4, margin kiri 4 cm dan sisi lain 3 cm, Times New Roman 12 pt, rata kiri-kanan, sitasi IEEE numerik, caption per BAB. Struktur **disederhanakan atas instruksi pengguna** — seluruh lembar formal (orisinalitas/persetujuan/pengesahan/publikasi), kata pengantar, abstract Inggris, daftar notasi/lampiran/riwayat hidup DIHAPUS karena ini tugas mata kuliah tanpa sidang; preseden penghapusan diambil dari proposal CARAKA untuk mata kuliah yang sama.
- **Yang tersisa sebelum penyusunan DOCX**: persetujuan bentuk final judul (usulan ada di `16_DOCUMENT_FORMAT_SPEC.md` §2, memasukkan kepanjangan AKSARA sesuai instruksi pengguna), pemetaan nama anggota ke pembagian modul, capture screenshot TUI, dan konfirmasi spasi baris 1,15 vs 1,5.
