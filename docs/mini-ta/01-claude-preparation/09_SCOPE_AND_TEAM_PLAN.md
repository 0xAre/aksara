...
> **KEPUTUSAN FINAL 2026-07-27** — judul yang dipakai bukan salah satu dari ketiga alternatif di bawah secara utuh, melainkan bentuk baru yang disetujui pengguna:
>
...
>
> Isinya tetap mengambil tiga pilar Judul #3, tapi diringkas dan diberi frasa pengait "Chat Terminal Tanpa Server". Konsekuensi dan larangan frasa terkait dicatat di `16_DOCUMENT_FORMAT_SPEC.md` §2. Analisis ketiga alternatif di bawah dipertahankan sebagai jejak pertimbangan.

**Rekomendasi (historis): Judul #3.** Alasan: (a) secara langsung merepresentasikan fokus utama §1 (satu rangkaian protokol: handshake + key lifecycle + threat model, bukan topik terpisah), sehingga tidak perlu "menyembunyikan" cakupan key lifecycle/threat model yang sudah dikerjakan penuh di TAHAP 5-7; (b) frasa "Implementasi dan Evaluasi Keamanan" (bukan "Desain" atau "Perancangan") jujur terhadap sifat pekerjaan — AKSARA sudah terimplementasi, tugas mini-TA adalah menganalisis dan mengevaluasi, bukan merancang dari nol; (c) "Aplikasi Chat Terminal P2P Serverless" secara akurat mendeskripsikan AKSARA tanpa mengklaim properti yang belum tentu benar (mis. tidak memakai kata "aman" secara mutlak, sesuai aturan anti-overclaim `CLAUDE_PREPARATION_BRIEF.md` aturan #16).
...
**Diperbarui 2026-07-27** dengan peran nyata yang disampaikan pengguna, menggantikan pembagian berbasis modul hasil asumsi SESSION 2. Ketiga peran bersifat fungsional — pengembangan, dokumentasi, dan pengujian — dan masing-masing memetakan ke bab laporan yang berbeda.

| Anggota | Peran | Tanggung Jawab Utama | Kaitan dengan Modul/Artefak | Bagian Laporan | Evidence Pendukung |
|---|---|---|---|---|---|
| **Andika Aryansyach Fauzan** (2322101878) | **Core developer — fullstack** | Perancangan dan implementasi seluruh codebase AKSARA v0.2.1: `identity/`, `crypto/`, `transport/`, `session/`, `contacts/`, `tui/`, `main.rs`. Menjadi narasumber teknis utama untuk seluruh klaim implementasi, dan memverifikasi bahwa setiap pernyataan di laporan benar-benar cocok dengan kode | Seluruh `src/` (7 modul, 46 unit test) | **BAB IV** (perancangan dan implementasi — bab inti, 7 subbab); verifikasi teknis BAB II dan BAB III | `01_CODEBASE_AUDIT.md`, `02_CRYPTO_IMPLEMENTATION_AUDIT.md`, `evidence/CODE_EVIDENCE_MATRIX.md`, seluruh `evidence/_raw-audit-json/` |
| **Mahendra Nur Hidayat** (2322101937) | **Dokumentasi akhir** | Penyusunan dokumen final: penerapan seluruh aturan `16_DOCUMENT_FORMAT_SPEC.md` (margin, spasi 1,15, penomoran `IV.1.1`, caption, footer), pembangkitan Daftar Isi/Gambar/Tabel otomatis, pengelolaan Daftar Pustaka IEEE 40 entry, serta konsistensi istilah dan pemiringan istilah asing lintas bab | Artefak dokumen, bukan modul kode | **BAB I**, **BAB II**, **BAB VI**, Abstrak, dan seluruh bagian awal/akhir | `16_DOCUMENT_FORMAT_SPEC.md`, `14_CHAPTER_CONTENT_PACK.md`, `references/REFERENCES.bib`, `13_TABLE_MANIFEST.md`, `11_FIGURE_MANIFEST.md` |
| **Rafi Putra Fadlurrahman** (2322101963) | **User testing** | Eksekusi dan pencatatan pengujian: menjalankan skenario EXP-01 s.d. EXP-05, memverifikasi perilaku aplikasi dari sisi pengguna (generate identitas, unlock vault, invite/fingerprint, sesi chat), mencatat hasil apa adanya ke CSV termasuk yang tidak sesuai harapan | Perilaku observable di boundary aplikasi (CLI dan TUI), bukan internal modul | **BAB V** (pengujian dan analisis, 3 subbab); bagian metodologi pengujian di **BAB III** | `12_TEST_PLAN.md`, `02-experiment-data/EXPERIMENT_RESULTS_2026-07-26.csv`, `EXPERIMENT_RESULTS_2026-07-27.csv`, `screenshots/STATUS.md` |

**Catatan pembagian**: pembagian ini **berbasis peran fungsional**, bukan pemecahan modul kode per orang. Struktur ini mencerminkan kondisi nyata — AKSARA sudah terimplementasi utuh oleh satu core developer sebelum penelitian dimulai, sehingga tugas mata kuliah ini berfokus pada mendokumentasikan, menguji, dan mengevaluasi artefak yang sudah ada. Memecah kepemilikan kode secara artifisial menjadi tiga hanya untuk terlihat seimbang justru akan menjadi klaim yang tidak benar, dan itu melanggar aturan kejujuran akademik `AGENTS.md` sekaligus Pasal 5 huruf a Perdir Pedoman TA.

Keseimbangan beban tetap terjaga karena ketiga peran menghasilkan luaran yang sebanding: implementasi menghasilkan artefak yang dianalisis, dokumentasi menghasilkan dokumen final yang dinilai, dan pengujian menghasilkan seluruh data BAB V. Ketiganya juga memetakan ke bab yang berbeda tanpa tumpang tindih.

> **Superseded**: versi sebelumnya subbab ini membagi tugas berdasarkan kepemilikan modul (`identity+crypto` / `transport+session` / `contacts+tui+integrasi`) dengan label `Anggota 1/2/3`. Pembagian itu adalah **asumsi perencanaan** SESSION 2 saat nama dan peran nyata belum diketahui. Diganti 2026-07-27 setelah pengguna menyampaikan peran sebenarnya. Label `Anggota 1/2/3` tidak lagi dipakai.
