...
Anggota kelompok memakai placeholder **Anggota 1**, **Anggota 2**, **Anggota 3** (status `NEEDS_CONFIRMATION`) — sudah diputuskan SESSION 2, dikonfirmasi ulang `PROGRESS.md` §Keputusan Penting poin 2: tidak ada nama asli yang dapat diverifikasi pada proposal CARAKA sebelumnya (seluruhnya placeholder `[Nama X — TBD]`).
...
Ketiga judul memakai nama panjang AKSARA (*Authenticated Key-based Secure Autonomous Relay Architecture*) hanya sebagai konteks penamaan proyek di BAB I, bukan bagian dari judul TA itu sendiri — konsisten dengan penamaan proyek di `AGENTS.md`.
...
| Nama anggota kelompok | `NEEDS_CONFIRMATION` | Placeholder Anggota 1/2/3 — tidak ada sumber nama asli yang dapat diverifikasi (SESSION 2) |
