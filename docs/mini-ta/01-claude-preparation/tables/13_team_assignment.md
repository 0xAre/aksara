...
Reproduksi tabel `09_SCOPE_AND_TEAM_PLAN.md` §8 sebagai artefak tabel siap-pakai BAB I/lampiran. Nama anggota memakai placeholder (status `NEEDS_CONFIRMATION`) — belum ada nama asli yang dapat diverifikasi.

| Anggota | Modul | Tugas Teknis | Eksperimen (rencana TAHAP 13) | Bagian Laporan |
|---|---|---|---|---|
| Anggota 1 | `src/identity/` (`keypair.rs`, `vault.rs`), `src/crypto/handshake.rs` (primitif) | Audit/verifikasi ulang inventarisasi primitif CORE-1..7; jelaskan generation kunci, layout vault, alur `seal()`/`unseal()`, parameter Argon2id | EXP-01 (correctness/rejection vault), EXP-05 (benchmark Argon2id) | BAB II (primitif kriptografi), BAB IV bagian identity/vault |
